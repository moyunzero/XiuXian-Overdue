...
async function runDay(
  act: (id: import('~/types/game').ActionId) => Promise<void>,
...
describe('CLASS-03 刷分 vs 打工冲突（D-13~D-16）', () => {
  ...
  it('D-13：连续纯刷分日达到 2 天后，系统进入偏科失衡态（边际递减可用）', async () => {
    const { game, act } = useGame()
    game.value = makeSeededGame()
    await runDay(act, 'study', 'study', 'tuna')
    await runDay(act, 'study', 'study', 'study')
  ...
  it('D-14：失衡后上课收益存在边际递减（相对无偏科基准）', async () => {
    const { game, act } = useGame()
  ...
  it('D-15：日志只提示失衡与参数调整，不出现策略指引用语', async () => {
    const { game, act } = useGame()
    game.value = makeSeededGame()
    await runDay(act, 'study', 'study', 'tuna')
    await runDay(act, 'study', 'tuna', 'study')
  ...
  it('D-16：连续偏科刷分时更可能出现现金链费用压力（费用池上升可观测）', async () => {
    const { game, act } = useGame()
