...
export type DayLogTone = 'info' | 'warn' | 'danger' | 'ok'
export type AddDayLog = (title: string, detail: string, tone?: DayLogTone) => void
...
/**
 * 推进一个游戏日：日息、日历、疲劳/专注、周结算（与 legacy `/game` endDay 一致）。
 */
export function rollCalendarDay(
  g: GameState,
  minPayment: number,
  applyWeeklyCollectionFeeFn: (g: GameState) => number,
  options: RollCalendarDayOptions = {}
): void {
  const addDayLog: AddDayLog = (title, detail, tone = 'info') => {
  ...
  if ((g.school.day - 1) % 7 === 0) {
    ...
    if (options.skipWeeklyExam) {
      g.logs.unshift({
        ...
        detail: `职场周度结算：债务压力：${debtPressure}；当前总债务：¥${currentTotalDebt.toLocaleString()}；${riskSummary}；${profileStatusNote}${profileChangeNote}。`,
