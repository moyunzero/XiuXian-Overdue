...
// ========== PSY-03：三轨解锁总结、冷数据快照（D-13～D-16） ==========

/** D-13：副指标阈值（驯化或麻木取高，与 D-05 一致） */
export const SUMMARY_SUBSIDIARY_THRESHOLD = 50

/** D-15③：累计游戏日解锁轨（非终局，仅解锁总结入口） */
export const SUMMARY_MIN_DAY_TRACK = 30

/** 叙事结局日志标题 — 与 makeNarrativeEndingEvent / resolveEvent 一致 */
export const NARRATIVE_ENDING_LOG_TITLE = '情节结局：麻木化时刻'

/** D-15②：与 data/events.json 及 PSY-02 卡组对齐的关键 collapse id */
export const KEY_COLLAPSE_EVENT_ID_FOR_SUMMARY = 'psy_collapse_spiral'

export function hasMetSummarySubsidiaryThreshold(g: GameState): boolean {
  return Math.max(g.domestication ?? 0, g.numbness ?? 0) >= SUMMARY_SUBSIDIARY_THRESHOLD
}

/** D-15②：首次「麻木化时刻」日志，或关键 collapse 已触发 */
export function hasSummaryNarrativeEndingKey(g: GameState): boolean {
  if (g.logs.some((l) => l.title === NARRATIVE_ENDING_LOG_TITLE)) return true
  const h = g.eventHistory?.[KEY_COLLAPSE_EVENT_ID_FOR_SUMMARY]
  return h !== undefined && h.times >= 1
}

/** D-15：三条件任一先到先解锁（纯函数，不含持久化旗标） */
export function shouldUnlockSummary(g: GameState): boolean {
  return (
    hasMetSummarySubsidiaryThreshold(g) ||
    hasSummaryNarrativeEndingKey(g) ||
    g.school.day >= SUMMARY_MIN_DAY_TRACK
  )
}

/** 与需求文档 hasMetSummaryThreshold 命名对齐 */
export const hasMetSummaryThreshold = hasMetSummarySubsidiaryThreshold
...
export function buildSummarySnapshot(g: GameState): SummarySnapshot {
  return {
    ...
    contractActive: g.contract.active,
    contractName: g.contract.name,
    contractProgress: g.contract.progress,
    contractVigilance: g.contract.vigilance,
...
export function makeNarrativeEndingEvent(): PendingEvent {
  return {
    title: NARRATIVE_ENDING_LOG_TITLE,
    body:
