...
> **状态**：Batch 1-5 核心功能已完成。详细实施状态见 `.monkeycode/docs/PLAN_A_SYSTEM_DESIGN.md`。
...
- [x] `app/composables/useGameState.ts`
  - 为默认存档补齐画像相关默认值
  - 保证旧档加载后不存在未初始化的画像字段

- [x] `app/composables/useGameStorage.ts`
  - 在 `loadFromSlot()` 中为历史存档补齐画像相关迁移逻辑
  - 为旧版本存档增加默认画像回填和安全兜底

### 2. 纯规则与画像推导层

- [x] `app/logic/gameEngine.ts`
...
- [x] 新增 `app/logic/gameEngine.profile.spec.ts`
  - 专门覆盖画像等级、标签生成和画像摘要
...
- [x] `app/composables/useGame.dayCycle.ts`
  - 在周结算阶段接入画像更新报告
  - 在周结算日志中增加"风险评级变化""标签变化""制度评估"摘要
  - 明确画像更新发生在日结前还是日结后，并固定顺序

- [x] `app/composables/useGame.economy.ts`
...
- [x] `app/composables/useGame.events.ts`
  - 扩展身体部位偿还文案和执行结果，使其反映画像标签
  - 已实现"身体抵押不只减债"扩展
  - 集成三种抵押类型（减债型/准入型/修行加速型）到身体偿还系统

- [x] `app/components/game/BorrowModal.vue`
  - 展示画像驱动的借贷解释，例如风险定价理由、额度压缩原因

- [x] `app/components/game/RepayModal.vue`
  - 展示系统对玩家当前偿付能力的制度判断摘要

- [x] `app/components/game/DebtDashboard.vue`
  - 增加风险评级、画像摘要或制度标签入口
...
- [x] 新增 `app/components/share/FateCard.vue`
  - 以制度报告卡而非战力卡为目标进行设计
  - 展示画像标签、制度结论和命运摘要

- [x] 新增 `app/components/share/FateCardGenerator.vue`
  - 从 `GameState` 或 `SummarySnapshot` 生成分享用结构化数据

- [x] 新增 `app/composables/useShare.ts`
  - 封装命运判定卡的导出与分享入口
...
- [x] `app/logic/gameEngine.ts`
  ...
  - 新增 `determineMortgageType()` 判定抵押类型
  - 新增 `calculateBodyMortgageBenefits()` 计算抵押收益
  - 新增 `applyBodyMortgageEffect()` 应用抵押效果
  - 新增 `canTriggerBodyMortgage()` 收紧触发条件
...
- [x] `app/types/game.ts`
  - 新增 `debtLock?: 'normal' | 'bodyLocked'` 字段
  - 新增 `lockedDebtAmount?: number` 字段

- [x] `app/logic/gameEngine.ts`
  - 新增 `isDebtLocked()` 检查债务是否锁定
  - 新增 `isCashRepayBlocked()` 检查现金还款是否被阻止
  - 新增 `getLockedDebtAmount()` 获取锁定债务金额
  - 修改 `shouldTriggerRepaymentEvent()` 增加触发条件检查
  - 修改 `applyBodyMortgageEffect()` 增加债务锁定逻辑

- [x] `app/composables/useGame.ts`
  - 修改 `repay()` 函数增加锁定债务检查
  - 修改 `resolveEvent` 中 `immediate_payment` 选项增加锁定检查

- [x] `app/logic/gameEngine.antiProfile.spec.ts`
  - 新增 17 个债务锁定相关测试用例
...
- [x] `app/logic/gameEngine.ts`
  ...
  - 新增 `isAntiProfileAction()` 判定反画像行为
  - 新增 `updateAntiProfileStreak()` 更新连续天数
  - 新增 `shouldTriggerAntiProfileRiskEvent()` 风险事件触发判定
  - 新增 `buildAntiProfileRiskEvent()` 生成风险事件
  - 新增 `applyAntiProfileConsequence()` 应用后果
...
- [x] `data/events.json`
  - 新增 `anti_profile_defiance_event` "系统误判"事件
...
- [x] 新增 `app/logic/gameEngine.antiProfile.spec.ts`
  - 覆盖反画像路线逻辑测试（27 个用例）
  - 覆盖债务锁定机制测试（17 个用例）

### 13. 文档与数据同步

- [x] `.monkeycode/docs/PLAN_A_SYSTEM_DESIGN.md`
  - 实施过程中同步更新已落地范围和偏差点

- [x] `.monkeycode/specs/plan-a-credit-society-2026/requirements.md`
  - 若玩法边界调整，同步更新需求范围

- [x] `.monkeycode/specs/plan-a-credit-society-2026/design.md`
  - 若画像结构或接入层变化，同步更新设计说明

## 里程碑建议

- [x] M1：完成画像类型、推导规则和基础测试
- [x] M2：完成画像驱动的借贷和事件分流
- [x] M3：完成游戏页画像展示与周结算画像报告
- [x] M4：完成制度归档和命运判定卡
- [x] M5：完成身体抵押增强（三种类型 + 债务锁定机制）与反画像路线首版
