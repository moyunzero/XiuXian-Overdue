...
- [x] 2. 实现预置种子库与导入逻辑
  - [x] 2.1 创建种子库数据文件
    - 在 `public/seed-events.json` 创建包含 20 高质量事件的文件
    - 事件内容符合"赛博朋克修仙"世界观，参考现实社会新闻改编
    - 覆盖多个 event family（催收、零工、修仙、制度、社交、推销、考试、健康）
    - 确保数值平衡（cash delta ≤ 5000, stat delta ≤ ±20, debt delta ≤ 总债务 30%）

  - [x] 2.2 实现种子库导入逻辑
    - 在 `useGame.ts` 中集成动态事件池初始化
